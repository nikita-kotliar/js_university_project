import{h as H}from"./assets/exercises_card-BEHkGXqB.js";import{i as f}from"./assets/vendor-I1I71QQ2.js";const r={filters:document.querySelector(".filters"),navButtons:document.querySelector(".nav-buttons"),musclesBtn:document.querySelector(".muscles-btn"),exercisesTitle:document.querySelector(".exercises-title"),searchForm:document.querySelector(".search-form"),loadMoreBtn:document.querySelector(".load-more-btn"),quoteContainer:document.querySelector(".quote"),pagination:document.querySelector(".pagination"),exercisesWrapper:document.querySelector(".exercises-div"),exercises:document.querySelector(".exercises-list")};let k=window.innerWidth<768?8:12,n=1,g="Muscles",q="Muscles",B="",b="",w="",m=[];O();x();r.musclesBtn.classList.add("active-btn");r.filters.addEventListener("click",I);r.exercises.addEventListener("click",W);r.searchForm.addEventListener("input",D);var M;(M=r.loadMoreBtn)==null||M.addEventListener("click",loadMore);async function x(e=!0){e&&(n=1,r.exercises.innerHTML="");let t=`https://your-energy.b.goit.study/api/filters?filter=${g}&page=${n}&limit=${k}`;q.trim()&&(t+=`&name=${q}`);const a=await(await fetch(t)).json();if(!a.results.length){F();return}j(a.results),C(a.totalPages)}function j(e){const t=`
    <ul class="exercises">
  ...
  `,r.searchForm.style.display="block",n=1,r.exercises.innerHTML="",await L())}async function L(e=!0){e&&(m=[]);let t=b.toLowerCase();t==="body parts"&&(t="bodypart");const s=`
    ...
    &page=${n}
    &limit=${k}
  `.replace(/\s+/g,""),i=await(await fetch(s)).json();if(!i.results.length){F();return}m=i.results,A(i.results),C(i.totalPages)}async function A(e){r.exercises.innerHTML="",m=e;const t=`
  ...
  `;r.exercises.insertAdjacentHTML("beforeend",t)}function D(e){B=e.target.value.trim().toLowerCase(),n=1,r.exercises.innerHTML="",L(!0)}function C(e){if(!r.pagination)return;if(e<=1){r.pagination.innerHTML="";return}let t="";const s=(o,u,v,d)=>`
    <li>
      <button 
  ...
  `;t+=s("left","icon-big",n-2,n<=2),t+=s("left","icon-small",n-1,n===1);let i=Math.max(1,n-1),l=Math.min(e,n+1);n===1&&(l=Math.min(e,3)),n===e&&(i=Math.max(1,e-2)),i>1&&(t+=a(1),i>2&&e>4&&(t+='<li class="dots">...</li>'));for(let o=i;o<=l;o++)t+=a(o);l<e&&(l<e-1&&e>4&&(t+='<li class="dots">...</li>'),t+=a(e)),t+=s("right","icon-small",n+1,n===e),t+=s("right","icon-big",n+2,n>=e-1),r.pagination.innerHTML=t}r.pagination.addEventListener("click",e=>{const t=e.target.closest(".pagination-btn");if(!t)return;const s=Number(t.dataset.page);s!==n&&(n=s,r.exercises.innerHTML="",b?L(!1):x(!1))});r.exercises.addEventListener("click",e=>{const t=e.target.closest('[data-action="start"]');if(!t)return;const s=m.find(a=>a._id===t.dataset.id);H(s)});function F(){var e;r.exercises.innerHTML=`
    <p class="no-results-paragraph">
      Unfortunately, <span>no results</span> were found.
    </p>
  `,(e=r.loadMoreBtn)==null||e.style.setProperty("display","none")}function p(e){return e[0].toUpperCase()+e.slice(1)}async function N(){return await(await fetch("https://your-energy.b.goit.study/api/quote")).json()}async function O(){try{const e=await N(),t=`
